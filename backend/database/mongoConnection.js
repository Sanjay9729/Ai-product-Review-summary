...
export async function connectToDatabase() {
  // If we already have a connected client and db, check if they are still valid
  if (client && db) {
    try {
      // Lightweight check to see if connection is still alive
      await db.admin().ping();
      return db;
    } catch (error) {
      console.warn('Existing connection lost, reconnecting...', error.message);
      client = null;
      db = null;
    }
  }
  ...
  try {
    ...
    const newClient = new MongoClient(MONGODB_URI, cloudOptions);
    
    try {
      await newClient.connect();
      client = newClient; // Assign only after successful connection
      ...
      const localClient = new MongoClient(localUri, localOptions);
