  ...
  try {
    if (intent === "generate-summaries") {
      ...
      const response = await fetch('http://localhost:3001/api/summaries', {
        ...
        body: JSON.stringify({ intent: 'generate-all-summaries' })
      });
...
export default function AISummaries() {
  ...
  const isLoadingSummaries = 
    ...
    fetcher.formData?.get("intent") === "generate-summaries";
  ...
  const generateAllSummaries = async () => {
    try {
      ...
      formData.append('intent', 'generate-summaries');
